...
export async function POST(request) {
    ...
    try {
        ...
        if (response.status === 201) {  // Check if the status is 201 Created
            return NextResponse.json({ domain: response.data.domain });
        } else {
            return NextResponse.json({ error: 'Failed to create store', details: response.data });
        }
    } catch (e) {
        console.error('Error creating store:', e.message);
        return NextResponse.json({ error: 'Failed to create store', details: e.message });
    }
}
